#!/usr/bin/env python3
"""Publish a narrowly constrained dependency update pull request to Forgejo."""
...
class Config:
    def __init__(
        self, server_url: str, repository: str, token: str, base_branch: str
    ) -> None:
    ...
    def from_env(cls, env: Mapping[str, str]) -> "Config":
        server_url = validate_server_url(env.get("FORGEJO_SERVER_URL", ""))
        repository = env.get("FORGEJO_REPOSITORY", "")
        ...
        if len(parts) != 2 or any(
            ...
            raise PublishError("FORGEJO_REPOSITORY must be an owner/name pair")
        token = env.get("FORGEJO_TOKEN", "")
        if not token:
            raise PublishError("FORGEJO_TOKEN is required")
        base_branch = env.get("BASE_BRANCH", "dev")
        validate_branch(base_branch, "BASE_BRANCH")
        return cls(server_url, repository, token, base_branch)
...
def validate_server_url(value: str) -> str:
    try:
        ...
        raise PublishError("FORGEJO_SERVER_URL is invalid") from exc
    if (
        parsed.scheme != "https"
        ...
        raise PublishError("FORGEJO_SERVER_URL must be an HTTPS origin")
...
class ForgejoClient:
    def __init__(self, config: Config) -> None:
        self.config = config
        self.api_root = f"{config.server_url}/api/v1"
        self.repo_path = "/".join(
        ...
        try:
            with self.opener.open(request, timeout=30) as response:
                if response.status not in expected:
                    raise PublishError(f"Forgejo API returned HTTP {response.status}")
            ...
            if not_found_ok and status == 404:
                return None
            raise PublishError(f"Forgejo API returned HTTP {status}") from None
        except urllib.error.URLError as exc:
            raise PublishError("Forgejo API request failed") from None
        except OSError:
            raise PublishError("Forgejo API request failed") from None
        if len(raw) > MAX_RESPONSE_BYTES:
            raise PublishError("Forgejo API response was too large")
        try:
            return json.loads(raw)
        except (UnicodeDecodeError, json.JSONDecodeError):
            raise PublishError("Forgejo API returned invalid JSON") from None
    ...
    def repository(self) -> Mapping[str, Any]:
        ...
        if not isinstance(value, dict) or not isinstance(value.get("id"), int):
            raise PublishError("Forgejo repository metadata was invalid")
        return value
    ...
    def branch(self, name: str) -> Mapping[str, Any]:
        ...
        if not isinstance(value, dict):
            raise PublishError("Forgejo branch metadata was invalid")
    ...
    def open_pulls_page(self, base: str, page: int) -> list[Any]:
        value = self.request(
            ...
            query={"state": "open", "base": base, "limit": PAGE_SIZE, "page": page},
        )
        if not isinstance(value, list):
            raise PublishError("Forgejo pull request metadata was invalid")
        return value
...
def has_open_update(client: ForgejoClient, repository_id: int, base: str) -> bool:
    for page in range(1, MAX_PAGES + 1):
        ...
        if len(pulls) < PAGE_SIZE:
            return False
    raise PublishError("Forgejo pull request pagination limit reached")
...
def run(args: argparse.Namespace, env: Mapping[str, str], root: Path) -> int:
    ...
    remote_head = branch_metadata["commit"]["id"]
    git = Git(root)
    if git.current_branch() != config.base_branch:
        raise PublishError("current branch does not match BASE_BRANCH")
    local_head = git.head()
    if local_head != remote_head:
        raise PublishError("local HEAD does not match the current Forgejo base branch")
